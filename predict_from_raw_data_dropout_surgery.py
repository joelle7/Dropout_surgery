##### Adapted from nnU-Net (https://github.com/MIC-DKFZ/nnUNet, pulled on 20-12-2023)
#original file: nnunetv2/inference/predict_from_raw data.py
#adjustments are indicated with 'CUSTOM'
...
from torchsummary import summary #custom: for model summary
...
class nnUNetPredictor(object):
    ...
    def _manage_input_and_output_lists(self, list_of_lists_or_source_folder: Union[str, List[List[str]]],
        ...
        print(f'There are {len(caseids)} cases that I would like to predict Joelle')
    ...
    def predict_from_data_iterator(self,
        ...
        """
        #print(self.network) #custom print model
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                ###*** CUSTOM ADDITION TO PERFORM MC DROPOUT ***###
                mc_passes = 20 #CUSTOM number of monte carlo dropout samples
                
                for idx_mc in range(mc_passes): #CUSTOM
                    print(f"Running iteration {idx_mc} of Monte Carlo dropout simulation for patient {os.path.basename(ofile)}")
                        ...
                        r.append(
                            export_pool.starmap_async(
                                ...
                                ((prediction, properties, self.configuration_manager, self.plans_manager,
                                  self.dataset_json, ofile, save_probabilities, idx_mc),) # CUSTOM idx_mc
                            )
                        )
                    else:
                ...
                try:
                    for params in self.list_of_parameters:
                        ...
                        if not isinstance(self.network, OptimizedModule):
                            for key in list(params.keys()): #custom
                                #print(key)
                                params[key.replace("all_modules.1.","all_modules.2.")] = params.pop(key) #custom
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        enable_dropout(self.network) #CUSTOM perform MC-dropout
...
### *** CUSTOM FUNCTION FOR MC-DROPOUT *** ###
def enable_dropout(model):
    ...
    for m in model.modules():
        ...
        if m.__class__.__name__.startswith('Dropout'):
            #print(f"found a dropout layer, counter is {counter}:")
            #print(m.__class__.__name__)
            if counter % 2 == 0:
                pass
            else:
                #print("counter is uneven")
                if 6 <= counter <= 16: #if i'm correct the encoder and decoder consist of 5 blocks, so the middle 5 blocks now contain dropout 
                    #print(f"set dropout layer {counter} to train mode")
                    m.train() #set dropout layer to train mode (i.e. activated)
            counter += 1
